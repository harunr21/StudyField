...
import { USERNAME_REGEX, type FriendshipWithProfile, type Profile } from "@/lib/types";
import { AlertCircle, Check, Loader2, MailPlus, UserMinus, UserPlus, Users, X } from "lucide-react";
...
export default function FriendsPage() {
    ...
    const reload = useCallback(async () => {
        const res = await listMyFriendships();
    ...
    const sendRequest = async () => {
        ...
        setSending(true);
        const result = await sendFriendRequestByUsername(normalized);
        if (result.error) {
            setError(result.error);
        } else {
            setInfo(result.info ?? "İstek gönderildi.");
            setUsernameInput("");
            await reload();
        }
        setSending(false);
    ...
    const incoming = items.filter((i) => i.direction === "incoming" && i.friendship.status === "pending");
    const outgoing = items.filter((i) => i.direction === "outgoing" && i.friendship.status === "pending");
    ...
    return (
        <div className="p-6 md:p-10 max-w-3xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-muted-foreground ml-[3px]">Arkadaşlarının oynatma listelerini ve ilerlemelerini gör.</p>
            </div>
                ...
                <Section title={`Gelen istekler (${incoming.length})`}>
                    {incoming.map((item) => (
                        <Row key={item.friendship.id} profile={item.profile}>
                            ...
                            <Button size="sm" variant="outline" onClick={() => remove(item.friendship.id)}>
...
function Section({ title, children, empty }: { title: string; children?: React.ReactNode; empty?: string }) {
    return (
        <div className="mb-6">
            <h2 className="text-sm font-semibold text-muted-foreground mb-2 uppercase tracking-wide">{title}</h2>
            {empty ? (
    ...
    return (
        <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-card p-3">
            ...
            <div className="min-w-0 flex-1">
                <div className="font-medium truncate">{profile.display_name?.trim() || `@${profile.username}`}</div>
